...
| `oss-contrib` | Contributor-side GitHub toolkit (`gh` CLI): find good-first issues (beginner-label synonyms, excludes already-PR'd/stale, optional curated seed), browse trending ∩ contributable, bootstrap fork→clone→branch, inventory merged-PR contributions (org vs external OSS, star-ranked, markdown badges), and stats (merge-rate, year/month/weekday, language). Terminal tables or dark/light HTML. Language/keyword/filters are all user args — no hardcoded domain. | `gh` (authenticated), `jq` |
