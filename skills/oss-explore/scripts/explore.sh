...
# GitHub 토픽은 소문자-하이픈 표기 → 키워드는 원문, 토픽은 슬러그로 변환
TOPIC_SLUG=$(printf '%s' "$TOPIC" | tr '[:upper:]' '[:lower:]' | tr ' ' '-')
...
KW=$(gh search repos "$TOPIC" "${common[@]}" 2>/dev/null || echo '[]')
    ...
    echo "$RESULT" | jq -r '
      ...
      (if .count == 0 then "_해당 주제로 발견된 레포가 없습니다. 키워드를 바꾸거나 --min-stars를 낮춰보세요._"
        ...
        ( "| 레포 | ★ | 언어 | 최근푸시 | GFI | HW | 설명 |",
          "|---|---|---|---|---|---|---|",
          (.repos[] | "| [\(.repo)](\(.url)) | \(.stars) | \(.language) | \(.pushed) | \(.gfi) | \(.hw) | \((.description)[0:55]) |") )
       else
        ( "| 레포 | ★ | 언어 | 최근푸시 | 설명 |",
          "|---|---|---|---|---|",
          (.repos[] | "| [\(.repo)](\(.url)) | \(.stars) | \(.language) | \(.pushed) | \((.description)[0:60]) |") )
      end),
